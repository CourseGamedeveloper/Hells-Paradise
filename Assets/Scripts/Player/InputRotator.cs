...
public class InputRotator : MonoBehaviour
{
    ...
    [SerializeField]
    float minHorizontalRotation = -360f;
    [SerializeField]
    float maxHorizontalRotation = 360f;

    [Tooltip("Rotate the object according to the mouse Y-axis movement?")]
    [SerializeField]
    bool verticalRotation = true;
    [SerializeField]
    float minVerticalRotation = -45f;
    [SerializeField]
    float maxVerticalRotation = 45f;

    [SerializeField]
    InputAction lookLocation = new InputAction(type: InputActionType.Value);
    ...
    void Update()
    {
        ...
        if (verticalRotation)
        {
            float newRotationX = mouseDelta.y * rotationSpeed;
